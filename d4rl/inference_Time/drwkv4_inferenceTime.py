...
if __name__ == "__main__":
    ...
    np.save('drwkv4_infTime.npy', data)
    np.save('drwkv4_memory.npy', memory_data)
    # loaded_data = np.load('drwkv4_infTime.npy')
    # # 将numpy数组转换回列表
    # x_loaded = loaded_data[0].tolist()
    # y_loaded = loaded_data[1].tolist()
    # # 绘制平均时间随序列长度增加的图
    # plt.plot(x_loaded, y_loaded, marker='o')
    # # 绘制平均时间随序列长度增加的图
    # plt.xlabel('Sequence Length')
    # plt.ylabel('Average Time Taken (seconds)')
    # plt.title('Average Time Taken vs. Sequence Length')
    # plt.grid(True)
    # plt.show()